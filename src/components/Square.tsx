import { printValueInSquare } from '../helpers/printValueInSquare'
...
const Square = ({ game, setGame, squareId }: SquareProps) => {
  ...
  return (
    <div className="square">
      {squareNumber.map((n: number, i: number) =>  <input type='number' className='number-input' value={printValueInSquare(n)} key={i + 1} onChange={(ev) => handleNumber(ev, i )}/>)}
    </div>
  )
}
